...
def get_energy_array(wave_data, nframes):
    ...
    for i in range(num):
        sum = 0
        for j in range(256 if i != num - 1 else (nframes % 256)):
...
def get_ZeroCrossingRate_array(wave_data, nframes):
    ...
    for i in range(num):
        sum = 0
        N = 256 if i != num - 1 else (nframes % 256)
        for j in range(N):
            m = i * 256 + j
            if m == 0:
                pass
...
# 将能力或过零率数组存成txt文件
def array_to_txt(feature_array, filepath, number):
    folder = os.path.exists(filepath)
    if not folder:                 
        os.makedirs(filepath)            
    filename = number + '_en.txt' if filepath == 'energy_file' else number + '_zero.txt'
    with open(filepath + '/' + filename, 'w') as f:
        for elements in feature_array:
            f.write(str(elements) + '\n')
    print ('成功保存:' + str(filename))
...
def ComputeThreshold(energy_array, ZeroCrossingRate_array):
    ...
    M_H = np.min(energy_array) + 0.03 * (np.max(energy_array) - np.min(energy_array))
...
def get_pcm(wave_data, label_lst, num, fileNumber):
    new_wave_data = []
    ...
    folder = os.path.exists('pcm/')
    if not folder:                 
        os.makedirs('pcm/')  
    new_wave_data.tofile('pcm/' + filename)
    print ('成功生成：' + filename)
...
if __name__ == '__main__':
    for i in range(10):
        ...
        #plt.plot(ZeroCrossingRate_array)
        #plt.show()
        array_to_txt(energy_array,'energy_file', filename[i][:-4])
        array_to_txt(ZeroCrossingRate_array,'zero_file', filename[i][:-4])
        ...
        frames_lable = end_point_detection(energy_array, ZeroCrossingRate_array,
                                           M_H, M_L, Z_s, num)
        get_pcm(wave_data, frames_lable, num, filename[i][:-4])
